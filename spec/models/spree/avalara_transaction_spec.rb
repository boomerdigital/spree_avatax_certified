...
describe Spree::AvalaraTransaction, :type => :model do
  ...
  describe "#lookup_avatax" do
    it "should look up avatax" do
      ...
      expect(@order.avalara_transaction.lookup_avatax["ResultCode"]).to eq("Success")
    end
  end
  ...
  describe "#commit_avatax" do
    it "should commit avatax" do
      @order.avalara_capture
      expect(@order.avalara_transaction.commit_avatax(@order.line_items, @order)["ResultCode"]).to eq("Success")
    end
  end
  ...
  describe "#commit_avatax_final" do
    it "should commit avatax final" do
      @order.avalara_capture
      expect(@order.avalara_transaction.commit_avatax_final(@order.line_items, @order, @order.number.to_s + ":" + @order.id.to_s, @order.completed_at)["ResultCode"]).to eq("Success")
    end
